...
function! fugitive#Head(...) abort
  ...
  if head =~# '^ref: '
    return substitute(head, '\C^ref: \%(refs/\%(for/\|remotes/\|tags/\)\=\)\=', '', '')
  elseif head =~# '^\x\{40,\}$'
    let len = a:0 ? a:1 : 0
...
function! s:Remote(dir) abort
  ...
  let i = 10
  while remote ==# '.' && i > 0
    let head = matchstr(fugitive#Config('branch.' . head . '.merge'), 'refs/for/\zs.*')
    let remote = len(head) ? fugitive#Config('branch.' . head . '.remote') : ''
    let i -= 1
  endwhile
...
function! s:CompleteRemote(A, L, P, ...) abort
  ...
  if !empty(remote)
    ...
    call map(matches, 's:sub(v:val, "^.*\t%(refs/%(for/|tags/)=)=", "")')
  else
...
function! fugitive#BufReadStatus() abort
  ...
  try
    ...
    if exists('rebasing_dir') && filereadable(rebasing_dir . 'git-rebase-todo')
      let rebasing_head = substitute(readfile(rebasing_dir . 'head-name')[0], '\C^refs/for/', '', '')
      let len = 11
...
function! s:DoStageUnpushedHeading(heading) abort
  ...
  call feedkeys(':Git push ' . remote . ' ' . '@:' . 'refs/for/' . branch)
endfunction
...
function! s:DoStageUnpushed(record) abort
  ...
  call feedkeys(':Git push ' . remote . ' ' . a:record.commit . ':' . 'refs/for/' . branch)
endfunction
...
function! fugitive#BrowseCommand(line1, count, range, bang, mods, arg, args) abort
  ...
  try
    ...
    for subdir in ['tags/', 'for/', 'remotes/']
    ...
    if path =~# '^\.git/refs/remotes/.'
      if empty(remote)
        ...
        let path = '.git/refs/for/'.merge
      endif
    elseif path =~# '^\.git/refs/for/.'
